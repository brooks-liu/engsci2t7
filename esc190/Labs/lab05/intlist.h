...
void create_list_from_data1(IntList **p_IntList, int *data_arr, int size){
    create_list_from_data(**p_IntList, *data_arr, int size);
}

void list_append1(IntList *list, int new_elem){
    testblock1(list);
    list_append(list, new_elem);
}

void list_insert1(IntList *list, int new_elem, int index){
    testblock1(list);
    list_insert(list, new_elem, index);
}

void list_delete1(IntList *list, int index){
    memmove(list->data + index, list->data + index + 1, list->size - index - 1);
    list->size -= 1;
}


void list_destroy1(IntList *list){
    list_destroy(list)
}


int list_get1(IntList *list, int index){
